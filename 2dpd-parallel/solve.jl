...
function init(setup_filename)
    ...
    println("PARALLELISM INFO")
    julia_nthread = Threads.nthreads()
    blas_nthread = BLAS.get_num_threads()
    println("\tjulia threads = $julia_nthread")
    println("\tblas threads = $blas_nthread")
    ...
    A, Pl, b, max_diagA = init_pressure_eq(P, dx, dy, sz, gc)
